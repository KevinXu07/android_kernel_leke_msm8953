...
static int elan_ktf_parse_dt(struct device *dev, struct elan_ktf_i2c_platform_data *pdata)
{
    ...
    // get interrupt gpio
    // get reset gpio
	/* L05 stock DTB uses elan,*; retain legacy aliases for other boards. */
	pdata->rst_gpio = of_get_named_gpio_flags(np, "elan,reset-gpio", 0,
						   &pdata->rst_gpio_flags);
	if (pdata->rst_gpio < 0)
		pdata->rst_gpio = of_get_named_gpio_flags(np, "elan_ktf,reset-gpio", 0,
							   &pdata->rst_gpio_flags);
    if (pdata->rst_gpio < 0)
	...
	if (pdata->intr_gpio < 0)
		pdata->intr_gpio = of_get_named_gpio_flags(np, "elan_ktf,irq-gpio", 0,
								&pdata->intr_gpio_flags);
    if (pdata->intr_gpio < 0)
